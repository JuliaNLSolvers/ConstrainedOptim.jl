...
abstract type ConstrainedOptimizer{T}  <: Optimizer end
abstract type IPOptimizer{T} <: ConstrainedOptimizer{T} end # interior point methods

struct OptimizationOptions{TCallback <: Union{Void, Function}}
    x_tol::Float64
    f_tol::Float64
    g_tol::Float64
...
mutable struct MultivariateOptimizationResults{T,N,M} <: OptimizationResults
    method::String
    initial_x::Array{T,N}
    minimizer::Array{T,N}
    minimum::Float64
